...
public:
    ...
    std::string toString() override {
        try {
            ...
            LOG_ERROR(LOG_NAME("system")) 	<< "ConfigVar::toString() exception" << e.what()
    ...
    bool fromString(const std::string& str) override {
        try {
            ...
            LOG_ERROR(LOG_NAME("system")) 	<< "ConfigVar::fromString() exception" << e.what()
											<< "convert: " << typeid(T).name();
        }
        return false;
    }
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& value,const std::string& description){
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
				LOG_ERROR(LOG_NAME("system")) << "lookup name=" << name
											  << " already exist";
				return tmp;
			}else{
				LOG_ERROR(LOG_NAME("system")) << "lookup name=" << name << " already exist but type not "
											  << typeid(T).name() << ", real type is " << it->second->getTypeName()
											  << ", value is " << it->second->toString();
				return nullptr;
            }
        }
        if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyz0123456789._")
                != std::string::npos){
            LOG_ERROR(LOG_NAME("system")) << "lookup invalid name " << name;
